...
<div class="card">
    ...
    <div class="card-body p-3">
        <form class ="needs-validation" action="<?php echo DIR; ?>modulo/update" method="post" data-action="update" enctype="multipart/form-data" autocomplete="off" novalidate>
            <?php foreach($modulo as $modulos): ?>
            <input type="hidden" id="ModuloID" name="ModuloID" value="<?php echo Main::encryption($modulos->ModuloID); ?>">
            <div class="row">
                <div class="col-2">
                    ...
                    <select name="PeriodoID" id="PeriodoID" class="form-select" aria-describedby="inputGroupPrepend" required>
                        <?php foreach($periodo as $row): ?>
                            <option value="<?php echo Main::encryption($row->PeriodoID); ?>" <?php echo ($modulos->PeriodoID == $row->PeriodoID)?'selected':''; ?>><?php echo $row->NombrePeriodo; ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-10">
                    ...
                    <select name="MaestriaID" id="MaestriaID" class="form-select" aria-describedby="inputGroupPrepend" required>
                        <?php foreach($maestria as $row): ?>
                            <option value="<?php echo Main::encryption($row->MaestriaID); ?>" <?php echo ($modulos->MaestriaID == $row->MaestriaID)?'selected':''; ?>><?php echo $row->NombreMaestria; ?></option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="row">
                <div class="col-10">
                    ...
                    <input type="text" name="NombreModulo" id="NombreModulo" class="form-control" maxlength="150" aria-describedby="inputGroupPrepend" value="<?php echo $modulos->NombreModulo; ?>" required>
